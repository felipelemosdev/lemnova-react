// src/contracts/templates/trabalhista.js
// Migrado de: trabalhista.pdf (enviado por Felipe em 01/09/2026) — versão completa, com
// a menção a Seguro-desemprego/FGTS/contribuições previdenciárias no Parágrafo Primeiro
// (confirmado por Felipe como a versão correta a usar).
//
// src/contracts/templates/doencaOcupacional.js é uma cópia deste arquivo com o tipo de
// ação trocado para "Doença Ocupacional" em todas as ocorrências — mesma estrutura,
// mesmas cláusulas, só o nome do tipo de ação muda (pedido explícito do Felipe).

import { clientFields } from "../../shared.js";

export function buildContratoTrabalhista(client) {
    ...
    return `
        ...
        <p><strong>Cláusula Primeira:</strong> O presente contrato tem como objeto a Prestação de Serviços de Assessoria Jurídica ao Contratante, com as seguintes medidas judiciais e/ou extrajudiciais: <strong>TRABALHISTA</strong>.</p>

        <p><strong>Cláusula Segunda dos Honorários: HONORÁRIOS – FORMA DE PAGAMENTO:</strong> A título de remuneração (honorários advocatícios) pela <strong>RECLAMAÇÃO TRABALHISTA</strong>, os contratados receberão do contratante, o valor correspondente a:</p>

        <p><strong>Parágrafo Primeiro:</strong> 30% (trinta por cento) do valor total da condenação que serão pagos de uma única vez no final da Reclamação Trabalhista proposta, ou o mesmo percentual do número de parcelas de eventual acordo formalizado em juízo OU FORA DELE, se for o caso, tudo após o efetivo recebimento. O valor incidirá em todas as verbas alcançadas pela sentença e pelo acordo, inclusive Seguro-desemprego e levantamento do FGTS, contribuições previdenciárias etc.</p>
